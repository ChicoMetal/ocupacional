...
class gosDB_Mysqli extends gosDB_Base
{
    ...
    {
        $this->conn = new mysqli($params->getHost(), $params->getUser(), $params->getPassword(), $params->getDBName(), $params->getPort());
    ...
    public function queryError($sql)
    {
         ...
         */
        $logger = Log5PHP_Manager::getLogger('gosdb.mysqli');
        $mysqlerrno = $this->ErrorNo();
        if ($mysqlerrno == 1205 || $mysqlerrno == 1213 && !($this->isInTransaction()))
        {
            $logger->info('A database error <' . $mysqlerrno . '> that appears to be a deadlock has occurred, retrying query<' . $sql . '>.');
            sleep(3);
            $resultSet = $this->conn->query($sql);
        }
        ...
        {
            $logger->info('A database error <' . $mysqlerrno . '> that appears to be a deadlock has occurred in a transaction, retrying all queries.');
            sleep(3);
        ...
        if (!$resultSet)
        {
            throw new gosException_DB('SQL Query failed: ' . $sql, get_defined_vars(), $sql, $resultSet, $this);
        }
    ...
    public function prepare($query)
    {
        ...
        {
            throw new gosException_DB('SQL Query failed', get_defined_vars(), $query, $stmt, $this);
        }
        return $stmt;
    }
    ...
    private function checkConnStatus()
    {
        ...
        throw new gosException_DB('Could not ping the db', get_defined_vars(), '(ping)', null, $this);
    }
