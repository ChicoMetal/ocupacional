...
class gosDB_AutoDBGenerator
{
     ...
     */
    private static $dbNames = array('main');
    ...
    {
        $command = 'mysql -u '. $connParams->getUser() .' -p'. $connParams->getPassword() .' -h '. $connParams->getHost() .' -P '. $connParams->getPort() ." -e '". $query ."'";
        $result = null;
        exec($command, $result);
        return $result;
    }
    ...
    protected function removeOldDatabases(gosDB_ConnParams $connParams)
    {
        ...
        $matches = explode('_', $connParams->getDBName());
        ...
        $result = $this->executeQuery($connParams, '
            ...
            where SCHEMA_NAME like "'. $dbName . '\_%"
        ...
        foreach ($result as $dbFullname)
        {
            ...
            if (gosRegex::match('/(' . $dbName . '_[a-z]+)_([0-9_]+)/', $dbFullname, $matches))
            {
                //var_dump($matches);
    ...
    {
        $cmd = 'mysql -u ' . $connParams->getUser() .' -p'. $connParams->getPassword() .' -h '. $connParams->getHost() .' -P '. $connParams->getPort() .' ' . $connParams->getDBName() . ' < ' . $filePath;
        //echo "executing $cmd\n";
        exec($cmd);
    }
}
