<script>
	$(document).ready(function() {
	 ...
	 function editar(codigo){
	 	...
	 	$("#load_edit_audiometrias_sugerencias").children().remove();
		$("#load_edit_audiometrias_sugerencias").load("<?php echo base_url()?>index.php/audiometrias_sugerencias/edit?codigo="+codigo);
	 	
	 }

	 function activar(codigo){
	 	$.ajax({
			url: '<?php echo base_url()?>index.php/audiometrias_sugerencias/cambiarestado',
			type: 'post',
	 ...
	 function desactivar(codigo){
			...
			<div class="box-content nopadding">
				<table class="table table-hover table-nomargin table-striped dataTable dataTable-reorder">
					...
					<tbody>
					<?php 
					if($audiometrias_sugerencias)
					foreach ($audiometrias_sugerencias as $data) {
						?>
