<script>
	$(document).ready(function() {
	 ...
	 function editar(codigo){
	 	...
	 	$("#load_edit_concepto_actitud_medica_ocupcional").children().remove();
		$("#load_edit_concepto_actitud_medica_ocupcional").load("<?php echo base_url()?>index.php/concepto_actitud_medica_ocupcional/edit?codigo="+codigo);
	 	
	 }

	 function activar(codigo){
	 	$.ajax({
			url: '<?php echo base_url()?>index.php/concepto_actitud_medica_ocupcional/cambiarestado',
			type: 'post',
	 ...
	 function desactivar(codigo){
			...
			<div class="box-content nopadding">
				<table class="table table-hover table-nomargin table-striped dataTable dataTable-reorder">
					...
					<tbody>
					<?php 
					if($concepto_actitud_medica_ocupcional)
					foreach ($concepto_actitud_medica_ocupcional as $data) {
						?>
